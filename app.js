...
quotes.forEach((quote) => {
  quote.addEventListener("click", function () {
    ...
    if (growableContainer !== null) {
      ...
      growableContentExpanded = growableContent.classList.contains(
        GROWABLE_CONTENT_EXPANDED_CLASS
      );
    }
      ...
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          if (entry.target.classList.contains("flipbook__page-back")) {
            entry.target
              .closest(".flipbook")
              .classList.add("flipbook--animate");
          } else {
...
class Tilt {
  ...
  update(e) {
    if (e.type == "mouseenter") {
      ...
      if (CSSTypedOM) {
        this.icon.attributeStyleMap.set(
          "transform",
      ...
      this.transform = {
        x: ((this.position.x - this.rect.x1 - this.rect.width) * 0.05).toFixed(
          1
        ),
        y: ((this.position.y - this.rect.y1 - this.rect.height) * 0.05).toFixed(
          1
        ),
      };
      ...
      if (this.isIntersecting) {
        if (CSSTypedOM) {
          const transform = new CSSTranslate(
            CSS.px(this.transform.x),
            CSS.px(this.transform.y),
            CSS.px(0)
          );
...
document.onreadystatechange = function () {
  if (
    ...
    var homeVideos = document.querySelectorAll(".misc video");
...
})(window, document, "script", "dataLayer", "GTM-5WC7GG7");
